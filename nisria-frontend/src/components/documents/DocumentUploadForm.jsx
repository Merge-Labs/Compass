...
const DIVISIONS = [
  ...
  { value: 'nisira', label: 'Nisria' },
  { value: 'maisha', label: 'Maisha' },
];

const DocumentUploadForm = ({ isOpen, onClose, onUploadSuccess, isDark }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      // The API expects 'document_link', not a file upload directly based on schema
      ...
      if (apiError && typeof apiError === 'object') {
        // Handle specific field errors from API if available
        const backendFieldErrors = {};
        let generalError = 'Upload failed. Please check the details and try again.';
      ...
      <div className={`rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto glass-panel ${isDark ? 'bg-gray-800/90 border border-gray-700' : 'bg-white/70 border border-white/40'}`}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            >
              {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Upload'}
            </button>
          </div>
        </form>
      </div>
