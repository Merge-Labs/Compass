...
import { X, Calendar, UserPlus, Loader2, Building } from 'lucide-react'; // Added Building
import api from '../../services/api';
import UserSelectionModal from './UserSelectionModal';
import { useTheme } from '../../context/ThemeProvider';
import GrantSelectionModal from './GrantSelectionModal'; // Import GrantSelectionModal

const UpdateTaskModal = ({ isOpen, onClose, onTaskUpdated, taskData }) => {
  ...
  useEffect(() => {
    if (isOpen && taskData) {
      setFormData({
        ...
        grant: taskData.grant || '',
      });
      setSelectedUserName(taskData.assigned_to_details?.full_name || 'Assign User');
      setSelectedGrantName(taskData.grant_details?.organization_name || 'Select Grant'); // Assuming grant_details might be available
      setErrors({});
  ...
  const validateForm = () => {
    ...
    if (formData.due_date && new Date(formData.due_date) < new Date().setHours(0,0,0,0) && new Date(formData.due_date).toISOString().split('T')[0] !== new Date(taskData.due_date).toISOString().split('T')[0]) {
        newErrors.due_date = "Due date cannot be in the past unless it's the original due date.";
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/api/tasks/${taskData.id}/update/`, payload);
      onTaskUpdated();
      onClose();
    } catch (error) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className={`rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
          <div className={`flex items-center justify-between p-4 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
            <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Update Task</h3>
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div>
              ...
              <input type="text" name="title" id="title" value={formData.title} onChange={handleInputChange} className={inputClasses} />
              {errors.title && <p className={errorTextClasses}>{errors.title}</p>}
            </div>

            <div>
              <label htmlFor="description" className={labelClasses}>Description</label>
              <textarea name="description" id="description" value={formData.description} onChange={handleInputChange} rows="3" className={inputClasses}></textarea>
            ...
            <div>
              ...
              <button type="button" onClick={() => setIsUserSelectionModalOpen(true)}
            ...
            <div className={`pt-4 border-t flex justify-end space-x-3 ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
              <button type="button" onClick={onClose} disabled={isSubmitting}
                className={`px-4 py-2 text-sm rounded-md border transition-colors ${isDark ? 'border-gray-600 text-gray-300 hover:bg-gray-700' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                Cancel
              </button>
              <button type="submit" disabled={isSubmitting}
                className={`px-4 py-2 text-sm rounded-md flex items-center justify-center font-medium text-white transition-colors ${isSubmitting ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                {isSubmitting ? (<><Loader2 size={16} className="animate-spin mr-2" />Updating...</>) : 'Update Task'}
              </button>
            </div>
          </form>
        </div>
