...
import { AlertCircle, User, Briefcase, Phone, Mail, Calendar as CalendarIcon, CheckSquare, Home, FileText, Gift, MessageSquare, BarChart, Star, Image as ImageIcon } from 'lucide-react';
...
const VocationalTraineeUpdateForm = ({ isOpen, onClose, existingBeneficiary, onBeneficiaryUpdated, programId, divisionName, trainerId, trainerAssociationFromProps }) => {
  ...
  useEffect(() => {
    if (isOpen && existingBeneficiary) {
      ...
      Object.keys(formData).forEach(key => {
        if (key !== 'pictures' && existingBeneficiary[key] !== undefined) {
          newFormData[key] = existingBeneficiary[key] || '';
        }
      });
      setFormData(newFormData);
      setImagePreview(existingBeneficiary.picture_url || null);
      setErrors({});
    }
  ...
  return (
    <FormContainer 
      title="Update Vocational Trainee"
      onClose={onClose}
      maxWidth="max-w-4xl"
    >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div><label htmlFor="age" className={labelClasses}>Age</label><div className="relative"><User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" /><input type="number" id="age" name="age" value={formData.age || ''} onChange={handleInputChange} className={`${inputClasses} pl-10`} /></div>{errors.age && <p className={errorClasses}><AlertCircle size={14}/>{errors.age}</p>}</div>
            <div>
          ...
          <div className="pt-4 mt-4 border-t border-gray-200">
            ...
            <div className="mt-5"><label htmlFor="background" className={labelClasses}>Background</label><textarea id="background" name="background" value={formData.background || ''} onChange={handleInputChange} rows="3" className={`${inputClasses} pl-4`}></textarea>{errors.background && <p className={errorClasses}><AlertCircle size={14}/>{errors.background}</p>}</div>
            <div className="mt-5"><label htmlFor="additional_support" className={labelClasses}>Additional Support</label><textarea id="additional_support" name="additional_support" value={formData.additional_support || ''} onChange={handleInputChange} rows="3" className={`${inputClasses} pl-4`}></textarea>{errors.additional_support && <p className={errorClasses}><AlertCircle size={14}/>{errors.additional_support}</p>}</div>
          </div>
          ...
          <div className="pt-4 mt-4 border-t border-gray-200">
            ...
            <div>
              ...
              <select id="post_training_status" name="post_training_status" value={formData.post_training_status || ''} onChange={handleInputChange} className={`${inputClasses}`}>
                <option value="">Select Status</option><option value="self_employed">Self-Employed</option><option value="employed">Employed</option><option value="further_education">Further Education</option><option value="unemployed">Unemployed</option><option value="unknown">Unknown</option>
              </select>
