...
import { X, AlertCircle, User, Briefcase, Phone, Calendar as CalendarIcon, CheckSquare } from 'lucide-react';

const initialFormData = {
  // Assuming fields for a trainee, adjust if trainer has different fields
  trainee_name: '', // or trainer_name
  course_enrolled: '',
  training_center: '',
  trainee_phone: '', // or trainer_phone
  start_date: '',
  expected_end_date: '',
  under_training: true,
...
const VocationalBeneficiaryForm = ({ isOpen, onClose, onBeneficiaryAdded, programId, divisionName }) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.trainee_name.trim()) newErrors.trainee_name = "Trainee/Trainer name is required.";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Assuming a generic endpoint for vocational beneficiaries.
      // If you have separate endpoints for trainees vs trainers, adjust this.
      const endpoint = `/api/programs/${divisionName.toLowerCase()}/vocational-trainers/`; // Or a more general vocational endpoint
      const response = await api.post(endpoint, payload);
      onBeneficiaryAdded(response.data);
      onClose();
    } catch (error) {
      console.error("Error adding vocational beneficiary:", error.response?.data || error.message);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h3 className="text-xl font-bold text-gray-800">Add Vocational Beneficiary</h3>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div>
            <label htmlFor="trainee_name" className={labelClasses}>Trainee/Trainer Name*</label>
...
export default VocationalBeneficiaryForm;