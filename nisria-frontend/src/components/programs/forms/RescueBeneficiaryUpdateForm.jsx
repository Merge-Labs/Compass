...
import { X, AlertCircle, User, Calendar as CalendarIcon, MapPin, Phone, Info, Users as GenderIcon, Loader2, Image as ImageIcon, FileText, Heart, Shield, BookOpen, Home, Briefcase } from 'lucide-react';
  ...
  const resetForm = (beneficiary) => {
    if (beneficiary) {
      ...
      Object.keys(formData).forEach(key => {
        if (key !== 'pictures' && beneficiary[key] !== undefined) {
          const value = beneficiary[key];
          // Format date fields for input type="date"
          if ((key === 'date_of_birth' || key === 'date_of_rescue' || key === 'date_of_exit') && value) {
            newFormData[key] = value.split('T')[0];
          } else {
