...
const DetailItem = ({ icon: Icon, label, value, highlight = false, isLongText = false }) => (
  <div
  ...
  >
    ...
    <dd className={`mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 ${isLongText ? 'whitespace-pre-wrap' : ''}`}>
      <span
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {!loading && !error && beneficiary && (
            <div className="space-y-6">
              <section>
                <SectionHeader icon={Heart} title="Child Information" />
                <DetailItem icon={User} label="Child's Name" value={beneficiary.child_name} highlight />
                <DetailItem icon={Calendar} label="Age" value={beneficiary.age} />
                <DetailItem icon={GenderIcon} label="Gender" value={beneficiary.gender} />
                <DetailItem icon={Calendar} label="Date Joined" value={formatDate(beneficiary.date_joined)} />
                <DetailItem icon={MapPin} label="Place Found" value={beneficiary.place_found} />
                <DetailItem icon={Phone} label="Rescuer Contact" value={beneficiary.rescuer_contact} />
                <DetailItem
                  icon={beneficiary.is_reunited ? CheckCircle : XCircle}
                  label="Reunited Status"
                  value={beneficiary.is_reunited ? "Reunited" : "Not Reunited"}
                />
                <DetailItem icon={Info} label="Circumstances" value={beneficiary.circumstances} isLongText/>
              </section>
